...
Rewards are granted on **Claim**. If deliver fails, the row stays **Ready** so you can retry. (not automatically when the goal is met).
...
- **Achievements tab hotkey** (default J)
- **Debug logging** OFF by default — turn ON only when diagnosing

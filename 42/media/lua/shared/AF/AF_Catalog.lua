...
function C.readSkillLevelProgress(player, modifierId)
    ...
    if modifierId == "none" then
        -- Any trainable skill EXCEPT Fitness/Strength (use cat.physical for those)
        local excludePhys = { fitness = true, strength = true }
        local best = maxLevelFiltered(player, function(perk, tname, pname)
            local tn = string.lower(tostring(tname or ""))
            if excludePhys[tn] then return false end
            return true
        end)
        -- also explicit common list in case factory walk fails (no Fitness/Strength)
...
function C.getModifiersForAction(actionId)
    ...
    if actionId == "skill_level" then
        local out = noneFirst({}, "(Any skill, not Fitness/Strength)")
...
function C.goalLabel(actionId, modifierId, amount)
    ...
    if modifierId == "none" then
        ...
        elseif actionId == "skill_level" then
            withPart = "in any skill (not Fitness/Strength)"
        else
            withPart = "any"
        end
    else

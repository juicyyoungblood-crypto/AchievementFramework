...
function C.readSkillLevelProgress(player, modifierId)
    ...
    if modifierId:sub(1, 4) == "cat." then
        ...
        for i = 1, #(cat.parents or {}) do
            parents[string.lower(cat.parents[i])] = true
        end
        local named = {}
        for i = 1, #(cat.perks or {}) do
            named[string.lower(cat.perks[i])] = true
        end
        if next(parents) ~= nil then
            local extra = maxLevelFiltered(player, function(perk, tname, pname)
                if named[string.lower(tname)] then return true end
                local pl = string.lower(pname or "")

    ...
    pcall(function()
        ...
        for i = 0, traits:size() - 1 do
            ...
            if def then
                ...
                -- Match vanilla "Good Trait" list: cost >= 0 (positive / free). Negatives are debuffs.
                if cost >= 0 then
                    local id = ""
                    -- Stable JSON id = CharacterTrait name (e.g. Graceful, Athletic)
                    pcall(function()
                        local t = def:getType()
                        if t then
                            if t.getName then id = tostring(t:getName() or "") end
                            if id == "" then id = tostring(t) end
                        end
                    end)
                    if id == "" then
                        pcall(function() if def.getType then id = tostring(def:getType()) end end)
                    end
                    -- strip Java tostring junk "zombie...CharacterTrait.Graceful" -> last segment
...
--- Resolve reward string to CharacterTrait (B42) or raw id
function R.resolveTrait(reward)
    reward = tostring(reward or "")
    if reward == "" then return nil, "empty" end
    -- CharacterTrait.GRACEFUL / CharacterTrait.Graceful style
    local found = nil
    pcall(function()
        if CharacterTrait then
            if CharacterTrait[reward] then found = CharacterTrait[reward]; return end
            local upper = string.upper(reward):gsub("%s+", "_")
            if CharacterTrait[upper] then found = CharacterTrait[upper]; return end
        end
    end)
    if found then return found, "CharacterTrait." .. reward end
    pcall(function()
        if CharacterTrait and CharacterTrait.valueOf then
            found = CharacterTrait.valueOf(reward)
        end
    end)
    if found then return found, "valueOf" end
    pcall(function()
        if CharacterTraitDefinition and CharacterTraitDefinition.getCharacterTraitDefinition then
            local def = CharacterTraitDefinition.getCharacterTraitDefinition(reward)
            if def and def.getType then found = def:getType() end
        end
    end)
    if found then return found, "definition" end
    -- pass through string for legacy getTraits():add
    return reward, "raw_string"
end
...
function R.deliver(player, def)
    ...
    if rtype == "trait" then
        ...
        pcall(function()
            if not traitObj then msg = "resolve_fail"; return end
            -- B42 path
            ...
            if not traits then msg = "no_traits"; return end
            local id = reward
            if type(traitObj) == "string" then id = traitObj end
            if traits.contains and traits:contains(id) then
                msg = "already_has_trait"
                ok = true
                return
            end

...
-- True if player already has this CharacterTrait enum (never pass strings).
local function playerHasTrait(player, traitObj)
    if not player or traitObj == nil or type(traitObj) == "string" then return false end
    local has = false
    pcall(function()
    ...
    pcall(function()
        if not player.getCharacterTraits then return end
        local ct = player:getCharacterTraits()
        if not ct then return end
        if ct.contains and ct:contains(traitObj) then
            has = true
            return
        end
    end)
...
local function removeTrait(player, traitObj)
    if not player or traitObj == nil or type(traitObj) == "string" then return false end
    local ok = false
    pcall(function()
        local ct = player:getCharacterTraits()
        if ct and ct.remove then
            ct:remove(traitObj)
            ok = true
        end
    end)
    pcall(function()
        if player.modifyTraitXPBoost then
            player:modifyTraitXPBoost(traitObj, true) -- remove boosts if API uses flag
        end
    end)
    -- Prefer verify: success if no longer has trait
    if not playerHasTrait(player, traitObj) then return true end
    return ok and (not playerHasTrait(player, traitObj))
end

local function traitDefCost(def)
    local c = 0
    pcall(function()
        if def and def.getCost then c = def:getCost() or 0 end
    end)
    return tonumber(c) or 0
end

--- Mutually exclusive CharacterTrait objects from a trait's definition (B42).
local function mutuallyExclusiveTraits(traitObj)
    local list = {}
    if traitObj == nil then return list end
    pcall(function()
        if not CharacterTraitDefinition then return end
        local def = nil
        if CharacterTraitDefinition.getCharacterTraitDefinition then
            def = CharacterTraitDefinition.getCharacterTraitDefinition(traitObj)
        end
        if not def and CharacterTraitDefinition.getTraits then
            local traits = CharacterTraitDefinition.getTraits()
            if traits and traits.size then
                for i = 0, traits:size() - 1 do
                    local d = traits:get(i)
                    local t = nil
                    pcall(function() t = d:getType() end)
                    if t == traitObj then def = d; break end
                end
            end
        end
        if not def or not def.getMutuallyExclusiveTraits then return end
        local mex = def:getMutuallyExclusiveTraits()
        if not mex then return end
        if mex.size then
            for i = 0, mex:size() - 1 do
                local x = mex:get(i)
                -- may be CharacterTrait or definition
                local tr = x
                pcall(function()
                    if x and x.getType then tr = x:getType() end
                end)
                if tr ~= nil and type(tr) ~= "string" then
                    list[#list + 1] = tr
                end
            end
        end
...
local function defForTrait(traitObj)
    local def = nil
    pcall(function()
    ...
    pcall(function()
        ...
        for i = 0, traits:size() - 1 do
            ...
            local t = nil
            pcall(function() t = d:getType() end)
            if t == traitObj then def = d; return end
        end
    end)
    return def
end

--- Grant positive trait reward (Knox-style):
--- 1) already has positive → success (claim OK)
--- 2) has mutually-exclusive negative (cost < 0) → remove negative INSTEAD of adding positive
--- 3) else add positive
local function grantPositiveTrait(player, traitObj, how)
    if not player or traitObj == nil then return false, "no_trait" end
    if type(traitObj) == "string" then return false, "string_not_trait" end

    if playerHasTrait(player, traitObj) then
        return true, "already_has_trait"
    end

    -- Remove opposing negatives instead of stacking the positive
    local removedAny = false
    local removedNames = {}
    local mex = mutuallyExclusiveTraits(traitObj)
    for i = 1, #mex do
        local other = mex[i]
        if playerHasTrait(player, other) then
            local odef = defForTrait(other)
            local cost = traitDefCost(odef)
            -- Negative / bad traits typically cost < 0 in B42 (vanilla Good Trait = cost >= 0)
            if cost < 0 then
                if removeTrait(player, other) then
                    removedAny = true
                    removedNames[#removedNames + 1] = tostring(other)
                    pcall(function()
                        if player.modifyTraitXPBoost then
                            player:modifyTraitXPBoost(other, false)
                        end
                    end)
                end
            end
    ...
    if removedAny then
        ...
        return true, "removed_negative:" .. table.concat(removedNames, ",")
    end
    ...
    if ok and playerHasTrait(player, traitObj) then
        return true, msg
    end
    if playerHasTrait(player, traitObj) then
        return true, "trait_present"
    end
    return ok, msg
end
...
function R.deliver(player, def)
    ...
    if rtype == "trait" then
        ...
        local ok, msg = grantPositiveTrait(player, traitObj, how)

...
function P.claim(player, signature)
    ...
    -- Refresh engine-backed scalars before qualify (works on server without client Track hooks)
    pcall(function()
        if not AF.Track or not AF.Track.getData then return end
    ...
    if not okDeliver then
        ...
        -- Halo: SP only. MP uses ClaimResult on client; server skips FX.
        local skipHalo = false
        pcall(function()
            if isClient and isClient() then skipHalo = true end
            if isServer and isServer() then skipHalo = true end
        end)
        if not skipHalo then
            pcall(function()
                if player.setHaloNote then

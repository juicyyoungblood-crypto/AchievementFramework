...
AF.VERSION = AF.VERSION or "0.15.0"
...
function Track.getData(player)
    ...
    -- one-time: peel vehicle out of total (pre-0.14.4 totals included car distance)
...
local function normalizeCraftSkill(raw)
    ...
    local s = lower(tostring(raw))
    s = s:gsub("^base:", ""):gsub("%s+", ""):gsub("_", ""):gsub("%-", "")
    s = s:gsub("%d+$", "")
    if s == "" then return nil end
    if CRAFT_SKILL_ALIAS[s] then return CRAFT_SKILL_ALIAS[s] end
    if #s >= 3 and #s <= 32 then return s end
    return nil
end
...
local function skillsFromCraftRecipe(cr, out, seen)
    ...
    pcall(function()
        if cr.getHighestRelevantSkill then
            local perk = cr:getHighestRelevantSkill()
            if perk then
                local nm = nil
                pcall(function() if perk.getName then nm = perk:getName() end end)
                pcall(function() if not nm then nm = tostring(perk) end end)

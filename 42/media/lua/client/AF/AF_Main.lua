...
AF.VERSION = AF.VERSION or "0.14.1"
...
function Track.getData(player)
    ...
    for _, n in ipairs({ "killed", "damage", "made", "eaten", "read", "fished", "distance" }) do ensure(d, n) end
...
local function playerHasSpeedDemon(player)
    local yes = false
    pcall(function()
        if CharacterTrait and CharacterTrait.SPEED_DEMON and player.hasTrait then
    ...
    if not yes then
        pcall(function()
            if player.hasTrait then
                if player:hasTrait("SpeedDemon") or player:hasTrait("speeddemon") then yes = true end
            end
        end)
    end
    return yes
...
local function maxDistanceSample(player, tickSpan)
    tickSpan = tickSpan or 15
    local maxKmh = S.maxVehicleKmh or 120
    pcall(function()
        local v = player and player.getVehicle and player:getVehicle()
    ...
    local sec = (tickSpan / 60.0) * mult
    local maxDist = tilesPerSec * sec * 1.5
    if maxDist < 2 then maxDist = 2 end
    if maxDist > 80 then maxDist = 80 end
    return maxDist
end
...
local function sampleDistance(player)
    ...
    local maxDist = maxDistanceSample(player, 15)
    -- reject teleport / glitch above 1.5x fastest car * speed demon for this window
    ...
    local inVeh = false
    pcall(function()
        if player.isSeatedInVehicle and player:isSeatedInVehicle() then inVeh = true end
        if not inVeh and player.getVehicle and player:getVehicle() then inVeh = true end
        if not inVeh and player.isDriving and player:isDriving() then inVeh = true end
    end)

    if inVeh then
        -- vehicle only — never sneak buckets in a car
        buckets[#buckets + 1] = "vehicle"
    else
        local aiming, sprint, running, sneaking = false, false, false, false
        pcall(function() if player.isAiming and player:isAiming() then aiming = true end end)
        pcall(function() if player.isSprinting and player:isSprinting() then sprint = true end end)
        pcall(function()
            if player.isRunning and player:isRunning() then running = true end
            if player.IsRunning and player:IsRunning() then running = true end
        end)
        pcall(function() if player.isSneaking and player:isSneaking() then sneaking = true end end)

        -- B42 has distinct aim-sneak state -> sneak_combat separate from standing combat.
        if aiming then
            if sneaking then
                            ...
                            pcall(function()
                                ...
                                if type(S.killedOnce) == "table" then
                                    ...
                                    for z, tmark in pairs(S.killedOnce) do
                                        ...
                                        pcall(function()
                                            if z and z.isDead and z:isDead() and (type(tmark) ~= "number" or tmark < tick - 300) then
                                                keep = false
                                            end
                                        end)

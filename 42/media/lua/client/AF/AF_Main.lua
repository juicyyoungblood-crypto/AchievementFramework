...
AF.VERSION = AF.VERSION or "0.15.12"
...
local function classifyLiterature(item)
    ...
    pcall(function()
        ...
        -- B42: Literature has NO hasTag(String) implementation — calling it throws
        -- "No implementation found" and Break-on-Error stops even inside pcall
        -- (SystemLore books hit this path). Never call item:hasTag here.
        ...
        if not isArmorMag then
            -- Knox System Lore paperbacks are books, not magazines
            if low:find("systemlore", 1, true) or low:find("knoxsystem.systemlore", 1, true) then
                bucket = "book"
                return
            end
            local taggedMag = false
            pcall(function()
                -- Prefer getTags() + contains only (no hasTag on Literature)
                local tags = nil
                local ok3, tgs = pcall(function()
                    if item.getTags then return item:getTags() end
                    return nil
                end)
                ...
                if tags then
                    ...
                    local okc3, c3 = pcall(function() return tags:contains("Magazine") end)
                    if okc3 and c3 then taggedMag = true end
                end
            end)

...
pcall(function() require "AF/AF_NetClient" end)
...
local function onGameStart()
    ...
    pcall(function()
        ...
        -- P1 MP: pull server player-defs + push local track
        if AF.Net then
            if AF.Net.requestDefsSync then AF.Net.requestDefsSync() end
            if p and AF.Net.requestTrackSync then AF.Net.requestTrackSync(p) end
        end
    end)
...
if Events.EveryOneMinute and AF and AF.Progress then
    Events.EveryOneMinute.Add(function()
        ...
        -- P1: periodic track mirror to server (no-op in pure SP)
        pcall(function()
            if AF.Net and AF.Net.requestTrackSync then
                local p = getSpecificPlayer and getSpecificPlayer(0) or getPlayer and getPlayer()
                if p then AF.Net.requestTrackSync(p) end
            end
        end)
    end)
end
